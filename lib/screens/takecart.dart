...
class TakeCart extends StatefulWidget {
  const TakeCart({Key? key}) : super(key: key);

  @override
  _TakeCartState createState() => _TakeCartState();
}

class _TakeCartState extends State<TakeCart> {
          ...
          children: [
            ...
            InkWell(
              onTap: ((){
                ...
                DocumentReference question = FirebaseFirestore.instance.collection('takeawaydetails').doc('$index');
                question.set(
                    {
                      "index": index,
                      "name": cartname,
  ...
  Future<void> fetch(int i) async {
    await FirebaseFirestore.instance.collection("takeawaydetails").get().then(
